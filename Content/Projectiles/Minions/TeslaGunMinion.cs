            ...
            if (AttackTimer <= 0)
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    ...
                    if (target != null && target.active && target.CanBeChasedBy())
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Projectile.Center.DirectionTo(target.Center) * 15 * (1 + NotAttackTimer / 60f), ModContent.ProjectileType<TeslaShotFriendly>(), (int)(Projectile.damage * (1 + NotAttackTimer / 60f)), 0.8f, Projectile.owner);
                        NotAttackTimer = 0;
